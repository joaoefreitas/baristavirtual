...
function agruparPorClassificacao(tarefas) {
  ...
  for (const nome of CLASSIFICACOES) {
    grupos[nome].sort(
      (a, b) => parseData(a.data_entrega) - parseData(b.data_entrega),
    );
  }
...
function renderizarGrupo(nome, tarefas) {
  ...
  const itens = tarefas
    .map(
      (t) =>
        `<li><h3>${t.materia}</h3><p>${t.descricao}</p><span class="prazo">Prazo: ${t.data_entrega}</span></li>`,
    )
    .join("");
...
function renderizarTarefas(tarefas) {
  ...
  const html = CLASSIFICACOES.map((nome) =>
